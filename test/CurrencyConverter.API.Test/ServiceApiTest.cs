...
namespace UnitTest.CurrencyLayer.Application
{
    public class ServiceApiTest
    {
        ...
        {
            var mock = new Mock<IHttpClient>();
            mock.Setup(x => x.GetBaseHttpTask<decimal>(Singleton.AppSettings.BaseUrl, "currency/currencies"));

            var service = new CurrencyConverterService(mock.Object);

            service.GetCurrencies();
            mock.Verify(x => x.GetBaseHttpTask<decimal>(Singleton.AppSettings.BaseUrl, "currency/currencies"), Times.Once);
        }
    }
}